...
@section('page-titles')
    <title>Update ChildCategory | Admin Dashboard </title>
@endsection
...
@section('body-content')
   <!-- pageheader  -->
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <h5 class="card-header">Update ChildCategories</h5>
                  <div class="card-body">
                    <form method="POST" action="{{ route('childCategory.update', $child_cat->id) }}">
                                  ...
                                  @foreach ($categories as $category)
                                      <option disabled>{{ $category->category_name }}</option>
                                        ...
                                        @foreach ( App\Models\SubCategory::where('category_id', $category->id)->get() as $subCat)
                                            <option value="{{ $subCat->id }}" @if( $child_cat->subcategory_id == $subCat->id ) selected @endif>---- {{ $subCat->subcategory_name }}</option>
                                        @endforeach
                                  @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input id="childCat_name" type="text" value="{{ $child_cat->childCategory_name }}" name="childCat_name" class="form-control" required autocomplete="off" placeholder="Write the ChildCategory name">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="input-select" name="status">
                                ...
                                <option value="1" @if( $child_cat->status == 1 ) selected @endif>Active</option>
                                <option value="2" @if( $child_cat->status == 2 ) selected @endif>Inactive</option>
                            </select>
                        </div>
